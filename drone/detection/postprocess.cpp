...
void dump_results(detection_result_t result) {
    printf(
        "detected obj=%d, conf=%.3f, coords=[(%d, %d), (%d, %d)]\n",
        result.obj_class, result.confidence,
...
static int process(uint8_t *input, std::vector<int> anchor, int grid_h, int grid_w, int stride,
...
static int process_u8(uint8_t *input, std::vector<int> anchor, int grid_h, int grid_w, int stride,
...
static int process_i8(int8_t *input, std::vector<int> anchor, int grid_h, int grid_w, int stride,
...
static int process_fp32(float *input, std::vector<int> anchor, int grid_h, int grid_w, int stride,
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
...
static int post_process_yolov5(std::vector<std::vector<int>> anchors, void *outputs,
                               rknn_tensor_attr *output_attrs, int model_in_h, int model_in_w,
                               float conf_threshold, float nms_threshold, float vis_threshold,
                               float scale_w, float scale_h, detection_result_group_t *group)
{
    ...
    for (size_t i = 0; i < anchors.size(); i++)
    {
        ...
        grid_h = output_attrs[i].dims[1];
        grid_w = output_attrs[i].dims[0];
        stride = model_in_h / grid_h;

        validCount += process((uint8_t *)_outputs[i].buf, anchors[i], grid_h, grid_w,
                              stride, filterBoxes, objProbs, classId, conf_threshold,
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        if (indexArray[i] == -1 || objProbs[i] < vis_threshold || i >= OBJ_NUMB_MAX_SIZE)
        {
            continue;
        }
        int n = indexArray[i];
        ...
        group->results[last_count].confidence = objProbs[n] * 100.f;
        group->results[last_count].obj_class = id;

#if DEBUG_RKNN
        dump_results(group->results[last_count]);
#endif
        last_count++;
    }
...
static int post_process_yolov7(std::vector<std::vector<int>> anchors, void *outputs, rknn_tensor_attr *output_attrs,
                               ...
                               float scale_w, float scale_h, detection_result_group_t *group)
{
    ...
    int grid_h = 0;
    int grid_w = 0;
    bool is_quant = (output_attrs[0].qnt_type == RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC && 
    ...
    for (size_t i = 0; i < anchors.size(); i++)
    {
        ...
        if (is_quant)
        {
            validCount += process_u8((uint8_t *)_outputs[i].buf, anchors[i], grid_h, grid_w, stride, filterBoxes, objProbs,
                                     classId, conf_threshold, output_attrs[i].zp, output_attrs[i].scale);
        } else {
            validCount += process_fp32((float *)_outputs[i].buf, anchors[i], grid_h, grid_w, stride, filterBoxes, objProbs,
        ...
        if (is_quant)
        {
            validCount += process_i8((int8_t *)_outputs[i].buf, anchors[i], grid_h, grid_w, stride, filterBoxes, objProbs,
                                     classId, conf_threshold, output_attrs[i].zp, output_attrs[i].scale);
        }
        else
        {
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        group->results[last_count].box.right = (int)(clamp(x2, 0, model_in_w) / scale_w);
        group->results[last_count].box.bottom = (int)(clamp(y2, 0, model_in_h) / scale_h);
        group->results[last_count].confidence = objProbs[i] * 100.f;
        group->results[last_count].obj_class = id;
...
int post_process(std::vector<std::vector<int>> anchors, void *outputs, rknn_tensor_attr *output_attrs,
                 model_type_e model_type, int model_in_h, int model_in_w,
                 float conf_threshold, float nms_threshold, float vis_threshold,
                 float scale_w, float scale_h, detection_result_group_t *group)
{
    int ret;
    ...
    {
    case YOLOV5:
        ret = post_process_yolov5(anchors, outputs, output_attrs, model_in_h, model_in_w,
                                  conf_threshold, nms_threshold, vis_threshold,
                                  scale_w, scale_h, group);
        break;
    case YOLOV7:
        ret = post_process_yolov7(anchors, outputs, output_attrs, model_in_h, model_in_w,
                                  conf_threshold, nms_threshold, scale_w, scale_h, group);
        break;
    default:
        ret = -1;
        break;
