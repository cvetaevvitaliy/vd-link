...
RK_S32 SAMPLE_COMM_ISP_Init(RK_S32 CamId, rk_aiq_working_mode_t WDRMode, RK_BOOL MultiCam, const char* iq_file_dir)
{
    ...
    SAMPLE_COMMON_ISP_SET_Flicker(CamId, 0); // 0: (60HZ); 1: (50HZ)
    return 0;
}
...
RK_S32 SAMPLE_COMM_ISP_SET_ManualExposure(RK_S32 CamId, float exposure_time)
{
    if (CamId >= MAX_AIQ_CTX || !g_aiq_ctx[CamId]) {
        printf("%s : CamId is over 3 or not init\n", __FUNCTION__);
        return -1;
    ...
    if (g_aiq_ctx[CamId]) {
        ...
        // Manual exposure mode, auto gain
        st_exp_sw_attr.AecOpType = RK_AIQ_OP_MODE_MANUAL;

        // Linear manual exposure: enable manual time, disable manual gain
        st_exp_sw_attr.stManual.stLinMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stLinMe.ManualGainEn = RK_FALSE;
        st_exp_sw_attr.stManual.stLinMe.TimeValue = exposure_time;

        // HDR manual exposure: set same time for all coefficients
        st_exp_sw_attr.stManual.stHdrMe.ManualTimeEn = RK_TRUE;
        st_exp_sw_attr.stManual.stHdrMe.ManualGainEn = RK_FALSE;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[0] = exposure_time;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[1] = exposure_time;
        st_exp_sw_attr.stManual.stHdrMe.TimeValue.fCoeff[2] = exposure_time;

        // Limit gain to reduce noise and prevent overexposure
        st_exp_sw_attr.stAuto.stLinAeRange.stGainRange.Min = 1.0f;   // Minimum 1×
        st_exp_sw_attr.stAuto.stLinAeRange.stGainRange.Max = 8.0f;   // Maximum 8.0×
...
#define AE_MATRIX_SIZE 15
#define AE_WEIGHT_BASE 32

void ae_apply_center_weight(Cam15x15UCharMatrix_t* matrix, int cx, int cy, int wx, int hy, int weight)
{
    for (int y = 0; y < AE_MATRIX_SIZE; ++y) {
        for (int x = 0; x < AE_MATRIX_SIZE; ++x) {
            int idx = y * AE_MATRIX_SIZE + x;
            if (x >= cx && x < cx + wx && y >= cy && y < cy + hy) {
                matrix->uCoeff[idx] = weight;
            } else {
                matrix->uCoeff[idx] = AE_WEIGHT_BASE;
            }
        }
    }
}

RK_S32 SAMPLE_COMM_ISP_SET_FastExposureRegion(RK_S32 CamId, int x, int y, int w, int h, uint8_t center_weight)
{
    if (CamId >= MAX_AIQ_CTX || !g_aiq_ctx[CamId]) {
        printf("%s : CamId is over max or not initialized\n", __func__);
        return -1;
    }
    ...
    ret = rk_aiq_user_api_ae_getExpSwAttr(g_aiq_ctx[CamId], &st_exp_sw_attr);
    if (ret != RK_AIQ_RET_SUCCESS) {
        pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
        return ret;
    }

    // Base + central emphasis
    ae_apply_center_weight(&st_exp_sw_attr.DayGridWeights,   x, y, w, h, center_weight);
    ae_apply_center_weight(&st_exp_sw_attr.NightGridWeights, x, y, w, h, center_weight);
...
RK_S32 SAMPLE_COMM_ISP_SET_FastAutoExposure(RK_S32 CamId, float min_time_s, float max_time_s, float max_gain)
{
    if (CamId >= MAX_AIQ_CTX || !g_aiq_ctx[CamId]) {
        printf("%s : CamId is over 3 or not init\n", __func__);
        return -1;
    ...
    if (g_aiq_ctx[CamId]) {
        ...
        // Run AE every frame for fast reaction
        st_exp_sw_attr.AecRunInterval = 0;
        st_exp_sw_attr.stAuto.stAeSpeed.DampOver          = 0.1f;
        st_exp_sw_attr.stAuto.stAeSpeed.DampUnder         = 0.1f;
        st_exp_sw_attr.stAuto.stAeSpeed.DampDark2Bright   = 0.05f;
        st_exp_sw_attr.stAuto.stAeSpeed.DampBright2Dark   = 0.05f;

        // Limit exposure time range to speed up transitions
        st_exp_sw_attr.stAuto.stLinAeRange.stExpTimeRange.Min = min_time_s;
        st_exp_sw_attr.stAuto.stLinAeRange.stExpTimeRange.Max = max_time_s;

        // Limit gain to reduce noise
        st_exp_sw_attr.stAuto.stLinAeRange.stGainRange.Min = 1.0f;
        st_exp_sw_attr.stAuto.stLinAeRange.stGainRange.Max = max_gain;
...
RK_S32 SAMPLE_COMMON_ISP_SET_AutoWhiteBalance(RK_S32 CamId, RK_BOOL bEnable)
{
    if (CamId >= MAX_AIQ_CTX || !g_aiq_ctx[CamId]) {
        printf("%s : CamId is over 3 or not init\n", __FUNCTION__);
        return -1;
    ...
    pthread_mutex_lock(&aiq_ctx_mutex[CamId]);
    if (g_aiq_ctx[CamId]) {
        ret = rk_aiq_uapi_setWBMode(g_aiq_ctx[CamId], bEnable ? OP_AUTO : OP_MANUAL);
    }
    pthread_mutex_unlock(&aiq_ctx_mutex[CamId]);
    return ret;
}
