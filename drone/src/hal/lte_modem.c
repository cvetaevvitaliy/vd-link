...
int lte_modem_get_signal_str(char* buf, size_t buf_sz)
{
    ...
    if (info.type[0] && strcmp(info.type, "lte") == 0) {
        ...
        if (info.rssi != LONG_MIN)
            n += snprintf(buf + n, buf_sz > (size_t)n ? buf_sz - (size_t)n : 0, " rssi %lddBm", info.rssi);
        if (info.rsrp != LONG_MIN)
            n += snprintf(buf + n, buf_sz > (size_t)n ? buf_sz - (size_t)n : 0, " rsrp %lddBm", info.rsrp);
        if (info.rsrq != LONG_MIN)
            n += snprintf(buf + n, buf_sz > (size_t)n ? buf_sz - (size_t)n : 0, " rsrq %lddB", info.rsrq);
        if (info.snr_valid)
            snprintf(buf + n, buf_sz > (size_t)n ? buf_sz - (size_t)n : 0, " snr %.1fdB", info.snr);
        return 0;
    }
